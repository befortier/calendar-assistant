  ...
  for (let i = 0; i < MAX_ITERATIONS; i++) {
    ...
    switch (result.stopReason) {
      ...
      case StopReason.ToolUse: {
        ...
        const proposalToolCalls = result.toolCalls.filter((tc) => isProposalTool(tc.name));
        const otherTools = result.toolCalls.filter((tc) => !isProposalTool(tc.name));

        if (proposalToolCalls.length > 0) {
          ...
          proposals.accumulate(proposalToolCalls);
          messages.push({ role: 'assistant', text: result.text, toolCalls: result.toolCalls });
          messages.push(...proposalToolCalls.map((tc) => ({
